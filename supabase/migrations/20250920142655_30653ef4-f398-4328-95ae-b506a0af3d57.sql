-- First, check and drop storage policies that depend on get_user_role
DROP POLICY IF EXISTS "Admins can delete report images in their council" ON storage.objects;
DROP POLICY IF EXISTS "Admins can view all report images in their council" ON storage.objects;

-- Now drop the problematic table policies
...
DROP FUNCTION IF EXISTS get_user_council(uuid) CASCADE;

-- Recreate security definer functions with fixed definitions
...
-- Profiles policies - only basic access
...
-- Admin can view profiles in same council (using function to avoid recursion)
...
-- Workers can access their assigned reports
CREATE POLICY "reports_worker_access" ON reports 
FOR SELECT USING (
...
CREATE POLICY "workers_citizen_view" ON workers 
FOR SELECT USING (
  id IN (
...
CREATE POLICY "completions_worker_access" ON work_completions 
FOR ALL USING (
  worker_id IN (SELECT id FROM workers WHERE user_id = auth.uid())
);

CREATE POLICY "completions_admin_view" ON work_completions 
FOR SELECT USING (
  public.get_user_role() = 'admin' AND
  worker_id IN (
